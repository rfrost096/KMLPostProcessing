...
def _create_kml_circle(latitude, longitude, altitude, radius_meters):
    ...
    for angle in range(0, 360, 1):  # Create points for the circle
        ...
        pts += [
            (longitude + x * METER_TO_DEGREE, latitude + y * METER_TO_DEGREE, altitude)
...
def _create_kml_rect_triangle(curr_pt, next_pt):
    ...
    rect_start_left = common.move_ecef_point_along_vec(
        rect_mid_left, trans_vec, DIRECTED_MARKER_LENGTH / 4.0
    )
    rect_start_right = common.move_ecef_point_along_vec(
        rect_mid_right, trans_vec, -DIRECTED_MARKER_LENGTH / 4.0
    )

    rect_end_left = common.move_ecef_point_along_vec(
        rect_mid_left, trans_vec, DIRECTED_MARKER_LENGTH / 4.0
    )
    rect_end_right = common.move_ecef_point_along_vec(
        rect_mid_right, trans_vec, DIRECTED_MARKER_LENGTH / 4.0
    )

    tip = common.move_ecef_point_along_vec(
        curr_pt_ecef,
    )
...
def generate_kml(options, additional_filters=None):
    ...
    if options.log_type == "per-cell-kpi":
        kpi_path = os.path.join(
            options.workarea, options.pci + "_" + options.kpi + ".csv"
        )
        ...
        (kpi_indices, gps_indices) = time_merger.merge(
            kpi_times, gps_times, options.merge_mode
        )
        ...
        elif options.pci and (
            "pci" in log_pd or (options.pci_col and options.pci_col in log_pd)
        ):
    ...
    kpi_min = (
        min(kpi_gps_pd[options.kpi]) if options.kpi_min == None else options.kpi_min
    )
    kpi_max = (
        max(kpi_gps_pd[options.kpi]) if options.kpi_max == None else options.kpi_max
    )
    ...
    for i in range(numRows):
        ...
        if "line" in options.draw_type:
            if i == numRows - 1:
                ...
                next_index = i + 1
                geom = kml.newlinestring(
                    coords=[
                        (lon, lat, alt),
                        (
                            kpi_gps_pd.iloc[i + 1]["longitude"],
                            kpi_gps_pd.iloc[i + 1]["latitude"],
                            kpi_gps_pd.iloc[i + 1]["altitude"],
                        ),
                    ]
                )
        ...
        if "point" in options.draw_type:
            ...
            geom.outerboundaryis = [
                (
                    lon - 0.5 * MARKER_WIDTH * METER_TO_DEGREE,
                    lat - 0.5 * MARKER_WIDTH * METER_TO_DEGREE,
                    alt,
                ),
                (
                    lon + 0.5 * MARKER_WIDTH * METER_TO_DEGREE,
        ...
        if "directed" in options.draw_type:
            ...
            poly_rear, poly_front = common.create_ref_directed_poly(
                center, 25, roll=roll, pitch=pitch, yaw=yaw, split=True
            )
            ...
            geom = kml.newlinestring(
                coords=[(lon, lat, alt + 5), (lon, lat, alt + 5 - vz * METER_TO_DEGREE)]
            )
            geom.altitudemode = simplekml.AltitudeMode.relativetoground
            geom.style.linestyle.color = kml_color
            geom.style.linestyle.width = LINE_WIDTH

            eom = kml.newlinestring(
                coords=[(lon, lat, alt + 5), (lon, lat + vx * METER_TO_DEGREE, alt + 5)]
            )
            geom.altitudemode = simplekml.AltitudeMode.relativetoground
            geom.style.linestyle.color = (
                kml_color / home / simran / Downloads / waste_gen_prediction.py
            )
            geom.style.linestyle.width = LINE_WIDTH

            eom = kml.newlinestring(
                coords=[(lon, lat, alt + 5), (lon + vy * METER_TO_DEGREE, lat, alt + 5)]
            )
            geom.altitudemode = simplekml.AltitudeMode.relativetoground
            geom.style.linestyle.color = kml_color
            geom.style.linestyle.width = LINE_WIDTH
        ...
        if "triangle" in options.draw_type:
            ...
            if common.lla_distance((lat, lon, alt), next_pt) < 0.5:
                geom.outerboundaryis = _create_kml_polygon(
                    lat, lon, alt, MARKER_WIDTH, 4
                )
            else:
            ...
            geom = kml.newpolygon()
            geom.altitudemode = simplekml.AltitudeMode.relativetoground
            geom.outerboundaryis = _create_kml_circle(
                next_pt[0], next_pt[1], next_pt[2], 1
            )
    ...
    if options.output_log:
        ...
        kml_fName = os.path.join(
            options.workarea, options.pci + "_" + options.kpi + ".kml"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Merged log name. Default is " + DEFAULT_LOG_NAME,
    )
    parser.add_argument(
        "-o",
        "--output-log",
        type=str,
        default=None,
        help="Output kml file. Default is the name of the KPI",
    )
    parser.add_argument(
        "-k",
        "--kpi",
        type=str,
        default=DEFAULT_KPI,
        help="KPI to plot. Default is " + DEFAULT_KPI,
    )
    parser.add_argument(
        "--kpi-min",
        type=float,
        default=None,
        help="Minimum value of the KPI color bar. Default is minimum of all KPIs.",
    )
    parser.add_argument(
        "--kpi-max",
        type=float,
        default=None,
        help="Maximum value of the KPI color bar. Default is maximum of all KPIs.",
    )
    parser.add_argument(
        "-p",
        "--pci",
        type=str,
        default=DEFAULT_PCI,
        help="PCI of the chosen cell. Default is " + DEFAULT_PCI,
    )
    parser.add_argument(
        "--use-pci-colormap",
        action="store_true",
        help="Specifies whether to use a provided PCI color map.",
    )
    parser.add_argument(
        "-w",
        "--workarea",
        type=str,
        default=DEFAULT_WORKFOLDER,
        help="Workarea path, containing radio, vehicle and traffic logs. Default is "
        + DEFAULT_WORKFOLDER,
    )
    parser.add_argument(
        "-u",
        "--kpi-units",
        type=str,
        default=DEFAULT_KPI_UNITS,
        help="KPI units. Default is dBm, for RSRP.",
    )
    parser.add_argument(
        "--draw-type",
        type=str,
        default=DRAW_TYPE,
        help="Draw type. Can be line or point or circle or any combination such as line,point or circle,point.",
    )
    parser.add_argument(
        "--pci-col", type=str, default=DEFAULT_PCI_COL_NAME, help="PCI Column name."
    )
    parser.add_argument(
        "--custom-label",
        type=str,
        default=None,
        help="A custom text to add to the pop-up label, at all data points.",
    )
    parser.add_argument(
        "--colormap",
        type=str,
        default=DEFAULT_COLOR_MAP,
        help="Colormap to use for the colorbar.",
    )
    parser.add_argument(
        "--labels",
        nargs="+",
        default=[],
        help="Fields to display in the pop-up label. Please also provide --label-units along with --labels.",
    )
    parser.add_argument(
        "--log-type", nargs="+", default=DEFAULT_LOG_TYPE, help="Input CSV log type."
    )
    parser.add_argument(
        "--alt-col", nargs="+", default=DEFAULT_ALT_COL, help="Altitude column."
    )
    parser.add_argument(
        "--use-discrete-colormap",
        action="store_true",
        help="Use a discrete color map, specified in the code, for KPI values",
    )
    parser.add_argument(
        "--alternate-colors",
        action="store_true",
        help="Change colors when the KPI changes",
    )
    parser.add_argument(
        "--label-units",
        nargs="+",
