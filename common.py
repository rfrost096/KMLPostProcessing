...
def create_ref_directed_poly(center_lla, size, roll=0, yaw=0, pitch=0, split=False):
    ...
    rect_south_west = (
        center_lla[0] - 0.25 * size * M_TO_DEGREE,
        center_lla[1] - 0.25 * size * M_TO_DEGREE,
        center_lla[2],
    )
    rect_south_east = (
        center_lla[0] - 0.25 * size * M_TO_DEGREE,
    ...
    tip_north = [
        center_lla[0] + 0.75 * size * M_TO_DEGREE,
        center_lla[1],
        center_lla[2],
    ]
    ref_poly = [
    ...
    if roll != 0:
        roll_line = [
            [
                (rect_south_east[0] + rect_south_west[0]) / 2.0,
                (rect_south_east[1] + rect_south_west[1]) / 2.0,
                (rect_south_east[2] + rect_south_west[2]) / 2.0,
            ],
            tip_north,
        ]
        roll_axis = create_3D_vec(roll_line[0], roll_line[1], normalize=True)
        roll_axis_ecef = create_3D_vec(
            lla_to_ecef(roll_line[0]), lla_to_ecef(roll_line[1]), normalize=True
        )
    ...
    if pitch != 0:
        pitch_line = [
            [
                (rot_poly[2][0] + rot_poly[1][0]) / 2.0,
                (rot_poly[2][1] + rot_poly[1][1]) / 2.0,
                (rot_poly[2][2] + rot_poly[1][2]) / 2.0,
            ],
            [
                (rot_poly[3][0] + rot_poly[4][0]) / 2.0,
                (rot_poly[3][1] + rot_poly[4][1]) / 2.0,
                (rot_poly[3][2] + rot_poly[4][2]) / 2.0,
            ],
        ]
        ...
        pitch_axis_ecef = create_3D_vec(
            lla_to_ecef(pitch_line[0]), lla_to_ecef(pitch_line[1]), normalize=True
        )
        for index, pt_ecef in enumerate(rot_poly_ecef):
            rot_poly_ecef[index] = rotate_point_3d(
                pt_ecef, pitch_axis_ecef, -pitch, center_ecef
            )
    ...
    if yaw != 0:
        pitch_line = [
            mid_point_3d(rot_poly_ecef[3], rot_poly_ecef[4]),
            mid_point_3d(rot_poly_ecef[1], rot_poly_ecef[2]),
        ]
        ...
        yaw_line_ecef = [
            center_ecef,
            move_ecef_point_along_vec(center_ecef, perp_ecef, 100),
        ]
        yaw_axis_ecef = create_3D_vec(
            yaw_line_ecef[0], yaw_line_ecef[1], normalize=True
        )

        for index, pt_ecef in enumerate(rot_poly_ecef):
            rot_poly_ecef[index] = rotate_point_3d(
                pt_ecef, yaw_axis_ecef, -yaw, center_ecef
            )
            rot_poly[index] = ecef_to_lla(rot_poly_ecef[index])
...
def ecef_to_lla(ecef_pt, flip_lat_lon=False):
    ...
    lon, lat, alt = pyproj.transform(
        ecef, lla, ecef_pt[0], ecef_pt[1], ecef_pt[2], radians=False
    )
    if flip_lat_lon:
        return [lon, lat, alt]
    else:
...
def rgb_to_kml_hex(rgb_dict):
    hex = "ff{:02x}{:02x}{:02x}".format(
        int(rgb_dict["b"] * 255.0),
        int(rgb_dict["g"] * 255.0),
        int(rgb_dict["r"] * 255.0),
    )
    return hex


def rgb_to_hex(rgb_dict):
    hex = "#{:02x}{:02x}{:02x}ff".format(
        int(rgb_dict["r"] * 255.0),
        int(rgb_dict["g"] * 255.0),
        int(rgb_dict["b"] * 255.0),
    )
...
def split_sequence(sequence):
    ...
    for i in range(min(left_len, right_len)):
        ...
        right_split_index = math.floor(
            0.5 * (sequence[mid_index + i] + math.floor(sequence[mid_index + i + 1]))
        )
